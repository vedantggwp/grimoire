...
## 2026-06-10 — v0.4.0 "The Living Grimoire": self-updating engine + Editorial Constellation frontend

**Decision:** Ship the update engine and the frontend overhaul as one release with two locked architectural choices. (1) Self-updates are **PR-gated and policy-driven**: `/grimoire:update` is a headless skill that never asks questions and never writes to the default branch — the two interactive taste checkpoints relocate to `_config/update.md` (standing judgment) and async PR review. The deterministic half follows the v0.3.1 hybrid-enforcement pattern: `lib/{update-policy,source-ledger,freshness,connections}.ts` emit evidence artifacts from compile (`freshness.json`, `connection-candidates.json`, `update-context.json`); the skill applies LLM judgment on top. The flagship trigger is a GitHub Actions cron workflow (`--setup` installs it); the engine is trigger-agnostic. (2) The frontend stays a **supercharged zero-dependency static site**: no framework, no CDN, file://-compatible — modern CSS (cross-document View Transitions, scroll-driven animations behind @supports, OKLCH categorical ramps) and small vanilla-JS modules deliver the award-level motion instead.

**Why:** The Karpathy LLM-wiki pattern Grimoire cites is "LLM-*maintained*" — v0.3.1 only built wikis, it never maintained them. A scheduled editorial pass (delta scout from the wiki's own open questions and coverage gaps, cross-run URL dedup, staleness telemetry, connection mining) completes the thesis, and PR review preserves SOUL principle 5 without blocking automation. On the frontend, the zero-dependency constraint is Grimoire's local-first soul; modern CSS got powerful enough that breaking it for a framework buys ceiling Grimoire doesn't need.

**Rejected alternatives:** auto-commit to the default branch (taste failures land unreviewed); a separate `dist/update.js` runtime (compile already owns artifact emission); an 8th MCP tool for staleness (coverage_gaps already answers "wiki health" — tool count stays at 7 per the wiki's own tool-consolidation guidance); an Astro rebuild of present (kills no-build-step/file://); a persistent update-state file (the ledger derives from approved-sources.md + raw/ frontmatter + log.md, so it can't drift).

**Trail:** Phases 0–4 landed as commit-sized chunks: present reorg (byte-identical css/ split verified by diff), correctness pass closing issues #1/#5/#6/#7/#8 (slug derivation now shared in `lib/slug.ts`, lockstep with papyr-core), update substrate (53 tests incl. tier boundaries with injected clocks), the update/scout skill layer (policy template round-trips through the real parser; claude-code-action inputs verified against current docs), design tokens + config engine (#3/#4/#9, AA contrast matrix across all 8 palettes × 2 themes), per-article routes + View Transitions (#2, legacy hash links redirect), hub constellation (deterministic FNV-seeded force layout — no Math.random, sites stay reproducible), reading experience (popovers, backlinks, freshness badges), and per-mode upgrades (gaps dropped its ~270KB inlined d3 for a build-time squarified treemap). 334/334 tests green; examples/mcp full site 628KB.
